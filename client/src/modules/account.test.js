...
describe('Account Actions', () => {
  ...
  function expectAccountsRequest() {
    expect(actions[0].type).toBe(FETCH_ACCOUNTS_REQUEST.toString());
  ...
  function expectAccountsSuccess(accounts) {
    expect(actions[1].type).toBe(FETCH_ACCOUNTS_SUCCESS.toString());
    expect(Object.keys(actions[1].payload).length).toBe(1);
    expect(actions[1].payload.data.items).toMatchObject(accounts);
    expect(actions[1].payload.data.count).toBe(accounts.length);
    expect(actions[1].payload.data.limit).toBe(50);
    expect(actions[1].payload.data.offset).toBe(0);
  }

  function expectAccountsError(msg) {
    expect(actions[1].type).toBe(FETCH_ACCOUNTS_ERROR.toString());
...
describe('Accounts Reducer', () => {
  ...
  it('should indicate when accounts are loading', () => {
    const state = reduce(undefined, FETCH_ACCOUNTS_REQUEST({ loading: true, data: {} }));
  ...
  it('should update state when accounts have errored', () => {
    ...
    const state = reduce(initialState, FETCH_ACCOUNTS_ERROR({ error: 'Oh Noes', data: {} }));
    expect(state.data).toMatchObject({});
