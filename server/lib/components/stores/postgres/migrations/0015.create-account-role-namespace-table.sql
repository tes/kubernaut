...
CREATE TABLE account_role_namespace (
  id UUID PRIMARY KEY,
  account UUID NOT NULL REFERENCES account ON DELETE CASCADE,
  role UUID NOT NULL REFERENCES role ON DELETE CASCADE,
  subject UUID REFERENCES namespace ON DELETE CASCADE,
  created_on TIMESTAMP WITH TIME ZONE NOT NULL,
  created_by UUID NOT NULL REFERENCES account,
  deleted_on TIMESTAMP WITH TIME ZONE,
  deleted_by UUID REFERENCES account,
  CONSTRAINT account_role_namespace__deletion__chk CHECK ((deleted_on IS NULL AND deleted_by IS NULL) OR (deleted_on IS NOT NULL AND deleted_by IS NOT NULL))
);

CREATE UNIQUE INDEX account_role_namespace__account__role__subject__uniq ON account_role_namespace (
  account DESC, role DESC, subject DESC
) WHERE deleted_on IS NULL;

CREATE INDEX account_role_namespace__account__idx ON account_role_namespace (
  account DESC
);

CREATE INDEX account_role_namespace__role__idx ON account_role_namespace (
  role DESC
);

CREATE INDEX account_role_namespace__subject__idx ON account_role_namespace (
  subject DESC
);
